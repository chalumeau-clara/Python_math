...
class NumericalIntegration:
    ...
    def set_nb_points(self):
        if n_text:
            integration_input[2] = int(n_text.get(0.0, END))
    ...
    def plot_function(self):
        ...
        """
        # Clear axes
        numerical_integration.ax.clear()
    ...
    def simpson_with_mapping(self, a, b, n, y_coords):
        ...
        if n % 2 != 0:
            print("The method can only be applied on an even number of points")
            return
        h = (b - a) / n
    ...
    def display_results(self):
        ...
        textstr = "\nRec inférieur: " + str(results['rectangle_inferieur']) + \
                  "\nRec suérieur: " + str(results['rectangle_superieur']) + \
        ...
        Label(self.label, text=textstr).pack(fill="both")
        plt.show()
...
def setup_interface(root):
    ...
    # Label for results.
    label = LabelFrame(frame, text="")
    label.grid(row=4, column=1, pady=10)
    # label.pack(fill="both", expand="yes")
    numerical_integration.set_label(label)
